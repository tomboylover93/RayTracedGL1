...
This README has instructions for building the RayTracedGL1 library on Linux. For the original README see README_ORIG.md.
...
```bash
git clone https://github.com/tomboylover93/RayTracedGL1
cd RayTracedGL1
cmake -S . -B build \
...
The build produces `build/libRayTracedGL1.so`. Set `RTGL1_SDK_PATH` to the repository root when building prboom-rt:
